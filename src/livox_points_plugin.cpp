...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    robo_ns = sdf->Get<std::string>("robotNamespace");
  if(robo_ns != "/")
    robo_ns = robo_ns + "/";
    ...
    body_link = sdf->Get<std::string>("bodyName");
 
   body_frame = robo_ns  + body_link;

    ROS_INFO_STREAM("body_frame:" << body_frame);
    node = transport::NodePtr(new transport::Node());
    node->Init(raySensor->WorldName());
    ROS_INFO_STREAM("raysensor world name:" << raySensor->WorldName());
    ROS_INFO_STREAM("raysensor parent name:" << raySensor->ParentName());
    ROS_INFO_STREAM("raysensor self name:" << raySensor->Name());
    ROS_INFO_STREAM("sdf self name:" << sdf->Get<std::string>("bodyName"));
    ...
    ROS_INFO_STREAM("my parent name:" << _parent->ParentName());
    ...
    ROS_INFO_STREAM("world  name:" << world->Name());
...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        SendRosTf(parentEntity->WorldPose(), body_frame, raySensor->Name());
        //ROS_INFO_STREAM("body_frame:" << body_frame);
	//SendRosTf(parentEntity->WorldPose(), raySensor->ParentName(), raySensor->Name());
	//ROS_INFO_STREAM("world  name:" << world->Name());
